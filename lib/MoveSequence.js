...
const normalizeMove = ({face, amount, width}, {down, left}) => {
	console.log("Trying to normalize move!")
	console.log(face + " " + amount + " " + width)
...
class MoveSequence {
	...
	toText({cross = null, rouxBlock = null, fixDirection = false, slices = []} = {}) {
		...
		if (rouxBlock === null) {
			...
			for (const move of this.moves) {
				console.log("Starting to nomalize")
				console.log(move)
		...
		return {
			text: moves.map((move) => getUniformNotationWithPrime(move)).join(' '),
			orientation: cubeOrientation,
		};
	}
	...
	push(move) {
		assert(faces.includes(move.latestMove.family));
		console.log(move)
		const newMove = {
			face: move.latestMove.family,
			amount: normalizeAmount(move.latestMove.amount),
